...
| [25](../25-fileless-lab/FINDINGS.md) | — (no BPF program) | — | *the odd one out*: a container lab on **fileless execution** detection. Builds and runs every listing from a threat-detection note — `memfd_create`+`execve`, `O_TMPFILE`, unlink+`fexecve`, pipe-to-interpreter — and checks the claimed `/proc/PID/exe` artifacts against what the kernel actually reports | not an eBPF technique at all — it is the *detection target* the earlier chapters would have to catch |
...
25. [Fileless execution, tested](../25-fileless-lab/FINDINGS.md) — the odd one out: **not** a new eBPF technique but a lab on **fileless execution detection**. A container builds and runs every code listing from a threat-detection note (`memfd_create`+`execve`, `O_TMPFILE`, unlink+`fexecve`, pipe-to-interpreter) and checks each claimed `/proc/PID/exe` artifact against what the kernel actually reports; the corrections that surfaced were folded back into the note. The document is published TLP:CLEAR — [read it online](https://douglasmun.github.io/ebpf101/25-fileless-lab/Adv_Linux_Threat_Detection_-_File_Identity_vs_Fileless_Execution_v1.3.html), with [review notes](../25-fileless-lab/review-file-identity-vs-fileless-v1_3.html) on its enforcement design. Findings, before/after trees and full run transcripts: [`FINDINGS.md`](../25-fileless-lab/FINDINGS.md)
