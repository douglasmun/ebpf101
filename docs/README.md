...
| [24](24-pythonbpf.md) | tracepoint (`sys_enter_execve`) | `HashMap` | ⚠️ *pre-1.0 dep, Docker-verified* — the kernel side written in **Python**, not C: `@bpf`/`@map`/`@section` lowered to LLVM IR via `llvmlite` → BPF `.o` → `pylibbpf`. Same hash-map execve-counter idea as ch02 (PID-keyed here, on the ch04 tracepoint), "third way" to author it | a pre-1.0 dependency ("not for production"); the *authoring model*, not a new hook or bridge |
