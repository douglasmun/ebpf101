...
# The verbatim v1.1 extraction is kept as the before-state for every finding:
...
exec docker run --rm "${FLAGS[@]}" \
  -v "$PWD/$SRC:/src:ro" -v "$PWD/tests:/tests:ro" -v "$PWD/detection:/d:ro" -v "$PWD/gate:/gate:ro" \
  fileless-lab bash -c '
