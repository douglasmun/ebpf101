...
| Q2 | How large a payload fits? | **32767 bytes** for the `user` type (bisected; 32768 → `EINVAL`). Static `./hello` is **704792 bytes — 21x over the ceiling.** A loader here would need a payload under 32 KB, a split across keys, or `big_key` (absent). |
